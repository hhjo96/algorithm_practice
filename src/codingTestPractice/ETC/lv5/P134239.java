...
// 콜라츠 추측이란 로타르 콜라츠(Lothar Collatz)가 1937년에 제기한 추측으로 모든 자연수 k에 대해
...
// 우박수 또는 우박수열로 불리기도 합니다. 현재 이 추측이 참인지 거짓인지 증명되지 않았지만 약 1해까지의 수에서
// 반례가 없음이 밝혀져 있습니다.
//은지는 우박수열을 좌표 평면 위에 꺾은선 그래프로 나타내보려고 합니다. 초항이 k인 우박수열이 있다면,
// x = 0일때 y = k이고 다음 우박수는 x = 1에 표시합니다. 이런 식으로 우박수가 1이 될 때까지
// 점들을 찍고 인접한 점들끼리 직선으로 연결하면 다음과 같이 꺾은선 그래프를 만들 수 있습니다.
...
// 0 이상의 수 b에 대해 [a, -b]에 대한 정적분 결과는 x = a, x = n - b, y = 0 으로 둘러 쌓인 공간의 면적으로
// 정의하며, 이때 n은 k가 초항인 우박수열이 1이 될때 까지의 횟수를 의미합니다.
//예를 들어, 5를 초항으로 하는 우박수열은 5 ⇒ 16 ⇒ 8 ⇒ 4 ⇒ 2 ⇒ 1 입니다. 이를 좌표 평면으로 옮기면
// (0, 5), (1, 16), (2, 8), (3, 4), (4, 2), (5, 1) 에 점이 찍히고 점들을 연결하면 꺾은선 그래프가 나옵니다.
// 이를 [0,0] 구간에 대해 정적분 한다면 전체 구간에 대한 정적분이며, [1,-2] 구간에 대해 정적분 한다면
// 1 ≤ x ≤ 3인 구간에 대한 정적분입니다.
...
public class P134239 {
    public static void main(String[] args) {
        ...
        System.out.println("solution(5, ranges) = " + Arrays.toString(solution(5, ranges)));
    }
    ...
    public static double[] solution(int k, int[][] ranges) {
        ...
        // 점의 개수
        int n = ys.size() - 1; // x=0 ~ x=n 까지, 총 n+1개의 점

        // 각 단위 구간 [i, i+1]의 넓이(사다리꼴 넓이)를 미리 계산
        // 사다리꼴 넓이 공식: (윗변+아랫변) * 높이 / 2
        // 여기서 높이는 1임
        double[] parts = new double[n];
        for (int i = 0; i < n; i++) {
            parts[i] = (ys.get(i) + ys.get(i + 1)) / 2.0;
        }
        ...
        for (int i = 0; i < n; i++) {
            // 0부터 i+1 번째까지의 누적 넓이
            prefix[i + 1] = prefix[i] + parts[i];
        }

        // 각 range에 대해 정적분 계산
        // b가 음수인경우 끝에서부터 세는 것
        // ex) b가 -2인경우, 끝에서 두번째 값임. x의최대값 + b 하면 됨
