...
// 게임의 목적은 다음 두 가지 조건을 만족시키면서, 한 기둥에 꽂힌 원판들을 그 순서 그대로 다른 기둥으로 옮겨서
// 다시 쌓는 것입니다.
//한 번에 하나의 원판만 옮길 수 있습니다.
//큰 원판이 작은 원판 위에 있어서는 안됩니다.
//하노이 탑의 세 개의 기둥을 왼쪽 부터 1번, 2번, 3번이라고 하겠습니다.
// 1번에는 n개의 원판이 있고 이 n개의 원판을 3번 원판으로 최소 횟수로 옮기려고 합니다.
//1번 기둥에 있는 원판의 개수 n이 매개변수로 주어질 때, n개의 원판을 3번 원판으로 최소로 옮기는 방법을
...
public class P12946 {
    ...
    public static void hanoi(int n, int from, int to, int mid) {
        ...
        // 1. n-1개를 from에서 mid로 옮김
        hanoi(n - 1, from, mid, to);

        // 2. 가장 큰 원판을 from에서 to로 옮김
        list.add(new int[]{from, to});

        // 3. n-1개를 mid에서 to로 옮김
